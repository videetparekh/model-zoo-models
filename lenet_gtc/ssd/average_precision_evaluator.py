...
from datasets.ssd_data_augmentations import Resize, RandomPadFixedAR, \
    ConvertTo3Channels, apply_inverse_transforms
...
from ssd.ssd_utils import iou
#from bounding_box_utils.bounding_box_utils import iou
import xxhash
import time
default_gt_format = {'class_id': 0, 'xmin': 1, 'ymin': 2, 'xmax': 3, 'ymax': 4}
default_pred_format = {'class_id': 0, 'conf': 1, 'xmin': 2, 'ymin': 3, 'xmax': 4, 'ymax': 5}
class Evaluator:
    '''
    ...
    def __init__(self,
                 ...
                 pred_format=None,
                 gt_format=None,
                 bit_precision=None, # ie. HP or LP
                 verbose=True):
        """
        Arguments:
        ...
        if pred_format is None:
            pred_format = default_pred_format
        if gt_format is None:
            gt_format = default_gt_format

        if not isinstance(data_generator, BatchGenerator):
            warnings.warn("`data_generator` is not a `DataGenerator` object, which will cause undefined behavior.")

        self.ssd_config = ssd_config
        n_classes = ssd_config.n_classes # +1 for background class
        self.model = model
    ...
    def __call__(self,
        ...
        self.compute_precision_recall(verbose=self.verbose, ret=False)
    ...
    def predict_on_dataset(self,
        ...
        #self.data_generator
        self.data_generator.build(**aug_dict,
        ...
        use_orig_decode = False
        #print('use_orig_decode', use_orig_decode)
        ...
        bit_precision_kwargs = {}
        if self.bit_precision is not None:
            assert self.bit_precision in ['hp', 'lp']
            bit_precision_kwargs = dict(hp= self.bit_precision=='hp')
        ...
        for j in tr:
            ...
            # Predict.
            t2 = time.time()
            y_pred = self.model.predict(batch_X, **bit_precision_kwargs)
            ...
            if self.model_mode == 'training':
                ...
                if use_orig_decode:
                    y_pred = ssd_utl.decode_detections_ssd_repo(
                         y_pred_raw,
                         confidence_thresh=self.ssd_config.score_thresh,
                         iou_threshold=self.ssd_config.nms_iou_thresh,
                         top_k=self.ssd_config.max_size_per_class,
                         input_coords=self.ssd_config.pred_coords,
                         normalize_coords=self.ssd_config.normalize_coords,
                         img_height=self.ssd_config.img_height,
                         img_width=self.ssd_config.img_width,
                         border_pixels=self.ssd_config.decoding_border_pixels)
                else:
                    y_pred = ssd_utl.decode_y(y_pred_raw, ssd_config=self.ssd_config)
    ...
    def match_predictions(self,
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            for i in tr:
                ...
                if gt.size == 0:
                    # If the image has no objects at all,
    ...
    def compute_precision_recall(self, verbose=True, ret=False):
        ...
        for class_id in range(1, self.n_classes + 1):
            ...
            eps = 1e-10
            n_gt = self.num_gt_per_class[class_id]
            cumulative_precision = np.where(tp + fp > 0, tp / (tp + fp + eps), 0) # 1D array with shape `(num_predictions,)`
            # a recall of -1 indicates that there were no ground truth labels for that class
            cumulative_recall =  np.where(n_gt > 0, tp / (n_gt+eps), 0) # 1D array with shape `(num_predictions,)`
    ...
    def compute_mean_average_precision(self, ret=True):
        ...
        # For COCO, use nan-mean: the classes with no data have NaNs:
        #mean_average_precision = np.average(self.average_precisions[1:]) # The first element is for the background class, so skip it.
        mean_average_precision = np.nanmean(self.average_precisions[1:])  # The first element is for the background class, so skip it.
